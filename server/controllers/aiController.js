...
async function generateCoverLetter(req, res) {
  try {
    ...
    // Fetch the job and ensure it belongs to this user
    const job = await Job.findOne({
      where: { id: jobId, user_id: userId },
    });
    if (!job) return res.status(404).json({ message: 'Job not found' });

    // Fetch the user’s email (used here as “name” placeholder)
...
Keep it to about 200 words, in a friendly but formal tone.
    ...
    // Call Cohere’s generate endpoint with a generate‐compatible model
    ...
    // Extract the generated text. Try both possible locations:
    let generations = [];
